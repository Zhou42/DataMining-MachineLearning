...
Yang - modified: line 305, line 622, line 256, line 812
...
class Pool(Op):
    ...
    def __init__(self, ds, ignore_border=False, st=None, padding=(0, 0),
        ...
        if mode not in ['max', 'average_inc_pad', 'average_exc_pad', 'sum', 'first_pool']:
            raise ValueError(
    ...
    def perform(self, node, inp, out):
        ...
        if self.padding != (0, 0):
            ...
            y = x
        func = numpy.max # modify this function to perform other type of pooling
        if self.mode == 'sum':
            func = numpy.sum
        elif self.mode == 'first_pool':
            func = Yang.first_pool
        elif self.mode != 'max':
            func = numpy.average
...
class PoolGrad(Op):
    ...
    def __init__(self, ds, ignore_border, st=None, padding=(0, 0), mode='max'):
        ...
        self.st = tuple(st)
        self.padding = tuple(padding)
        if mode not in ['max', 'sum', 'average_inc_pad', 'average_exc_pad', 'first_pool']:
            raise ValueError(
                "Pool mode parameter only support 'max', 'sum',"
                " 'average_inc_pad' and 'average_exc_pad'. Got %s" % mode)
        self.mode = mode
...
class AveragePoolGrad(PoolGrad):
    def __init__(self, ds, ignore_border, st=None, padding=(0, 0),
        ...
        assert mode in ['sum', 'average_inc_pad', 'average_exc_pad','first_pool']
